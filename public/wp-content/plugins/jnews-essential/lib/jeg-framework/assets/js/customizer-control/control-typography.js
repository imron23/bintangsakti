wp.customize.controlConstructor['jeg-typography'] = wp.customize.controlConstructor.default.extend({
	...
	ready: function() {
		...
		var control                = this,
			...
			fontSizeUnitSelector   = control.selector + ' .font-size select',
			lineHeightUnitSelector = control.selector + ' .line-height select',
			hasDefault             = false,
		...
		// font-size-unit
		jQuery( fontSizeUnitSelector ).selectize();
		this.container.on( 'change', '.font-size select', function() {

			// Add the value to the array and set the setting's value.
			value['font-size-unit'] = jQuery( this ).val();
			control.saveValue( value );
		});

		// line-height-unit
		jQuery( lineHeightUnitSelector ).selectize();
		this.container.on( 'change', '.line-height select', function() {

			// Add the value to the array and set the setting's value.
			value['line-height-unit'] = jQuery( this ).val();
			control.saveValue( value );
		});
