...
Author: vafpress
Author URI: #
Text Domain: vafpress-post-formats-ui-develop
*/
...
add_action( 'plugins_loaded', function() {
	load_plugin_textdomain( 'vafpress-post-formats-ui-develop', false, 'vafpress-post-formats-ui-develop' . '/languages/' );
} );
...
function vp_pfui_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		...
		wp_localize_script(
			...
			'vp_pfui_post_format',
			array(
				'loading'      => __('Loading...', 'vafpress-post-formats-ui-develop'),
				'wpspin_light' => admin_url('images/wpspin_light.gif'),
				'media_title'  => __('Pick Gallery Images', 'vafpress-post-formats-ui-develop'),
				'media_button' => __('Add Image(s)', 'vafpress-post-formats-ui-develop')
			)
		);
