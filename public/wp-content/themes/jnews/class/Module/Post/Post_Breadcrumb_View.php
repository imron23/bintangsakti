...
Class Post_Breadcrumb_View extends PostViewAbstract
{
    ...
    {
        return
            "<div {$this->element_id($attr)} class='jeg_custom_breadcrumb_wrapper {$attr['scheme']} {$attr['el_class']} {$this->get_vc_class_name()}'>
    ...
    public function render_module_front($attr, $column_class)
    {
        ...
        return "<div {$this->element_id($attr)} class='jeg_custom_breadcrumb_wrapper {$attr['scheme']} {$attr['el_class']} {$this->get_vc_class_name()}'>" . $single->render_breadcrumb(false) . "</div>";
    }
}